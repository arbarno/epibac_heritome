...
def pick_longest_mRNA(gff_details):
    ...
    for s in gff_details:
        for g in gff_details[s]:
            # Check if any mRNA exists for this gene.
            if not gff_details[s][g].mRNAs:
                continue  # Skip genes with no mRNA entries

            # Sort mRNAs by longest exonic length (using 'exon' details) and then by total mRNA length.
            sorted_mRNA = sorted(
                gff_details[s][g].mRNAs,
                key=lambda x: (
                    -calc_total_exonic_length(gff_details[s][g].mRNAs[x].details.get('exon', [])),
                    -len(gff_details[s][g].mRNAs[x])
                )
            )

            # Only update if the sorted list is not empty.
            if sorted_mRNA:
                gff_details[s][g].mRNAs = {
                    sorted_mRNA[0]: gff_details[s][g].mRNAs[sorted_mRNA[0]]
                }
    return gff_details
