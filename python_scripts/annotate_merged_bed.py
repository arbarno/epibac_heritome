...
parser.add_argument('merged_filtered_bed', metavar="bed_filename",
                    type=argparse.FileType('r'), nargs='?',
                    default=sys.stdin, help="Merged filtered .bed filename.")
...
for s in natural_sort.natural_sort(scaffold_gff3):
    ...
    for gene_id in natural_sort.natural_sort(scaffold_gff3[s]):
        ...
        if not scaffold_gff3[s][gene_id].mRNAs:
            print(f"Warning: No mRNA for gene {gene_id} on {s}", file=sys.stderr)
            continue  # or handle it in a way that's appropriate for your analysis
...
tsv_reader = csv.reader(args.merged_filtered_bed, delimiter='\t')
counter = 0
for row in tsv_reader:
    ...
    output = row[:3] + [str(round(meth/valid * 100, 4))] + row[4:6]
